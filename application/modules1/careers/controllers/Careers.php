    ...
    function jobApply()
    {
       ...
       /*$ci = CI::get_instance();
            ...
            if (!$ci->upload->do_upload('uploadfile')) {
                ...
                if ($data['jobapplyid']=$this->Mdl_careers->jobsApply()) {
                    /*setInformUser('success', ' Successfully Completed Job Applications.');*/
                    $this->session->set_userdata('jobapplyid',$data);
                    redirect(base_url('careers/jobApplyStep2/'));
    ...
    public function jobApplyStep2(){
             ...
             if(strtolower($_SERVER['REQUEST_METHOD']) == 'post') {
                 ...
                 if (!$ci->upload->do_upload('uploadfile')) {
                     ...
                     $file = $data['upload_data']['file_name'];
                     if ($this->Mdl_careers->jobsApplyStep2($file)) {
